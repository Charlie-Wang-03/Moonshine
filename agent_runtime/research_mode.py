...
from moonshine.utils import overlap_score, read_text, shorten, slugify, trim_text_to_token_budget, utc_now


PROJECT_RESOLUTION_INPUT_TOKEN_BUDGET = 100000
...
def build_research_mode_policy(project_slug: str, agent_slug: str = "") -> str:
    ...
    lines = [
        ...
        "- `workspace/problem.md` is the current problem reference when needed; use `read_runtime_file` or MCP filesystem tools to obtain resolved paths and file contents.\n"
...
class ResearchProjectResolver(object):
    ...
    def resolve(self, user_message: str) -> ResearchProjectResolution:
        ...
        if self.provider is not None and not isinstance(self.provider, OfflineProvider) and hasattr(self.provider, "generate_structured"):
            try:
                bounded_user_message = trim_text_to_token_budget(user_message, 70000)
                bounded_projects = trim_text_to_token_budget(
                    json.dumps(projects, ensure_ascii=False, indent=2),
                    PROJECT_RESOLUTION_INPUT_TOKEN_BUDGET - 70000,
                )
                prompt = (
                    ...
                    "First user message:\n%s\n\n"
                    "Existing projects:\n%s"
                ) % (bounded_user_message, bounded_projects)
